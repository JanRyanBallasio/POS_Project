...
function Receipt({ selectedCustomer, cartTotal }) {
    const { cart } = (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$contexts$2f$cart$2d$context$2e$tsx__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["useCart"])();
    async function downloadPdf() {
        const API_BASE = ("TURBOPACK compile-time value", "http://localhost:5000/api") && ("TURBOPACK compile-time value", "http://localhost:5000/api").replace(/\/$/, "") || "http://localhost:5000/api";
        // build items payload from cart
        const items = (cart || []).map((ci)=>{
            const price = Number(ci.product?.price || 0);
            const qty = Number(ci.quantity || 0);
            const desc = ci.product && (ci.product.name || ci.product.title) || ci.product?.barcode || "Item";
            return {
                desc,
                qty,
        ...
        try {
            ...
            if (!res.ok) {
                ...
                console.error("PDF request failed:", res.status, res.statusText, text);
                return;
            }
...
const styles = __TURBOPACK__imported__module__$5b$externals$5d2f40$react$2d$pdf$2f$renderer__$5b$external$5d$__$2840$react$2d$pdf$2f$renderer$2c$__esm_import$29$__["StyleSheet"].create({
    ...
    // Description: flexible column, can shrink and wrap inside its box
    colDesc: {
        flexBasis: 0,
        flexGrow: 1,
        flexShrink: 1,
        minWidth: 0,
        // reserve horizontal room so the description cannot flow under Qty/Amount
        paddingRight: 92,
        // break very long continuous strings so they wrap inside the column
        wordBreak: "break-all"
    },
    // Fixed widths for Qty and Amount so Description cannot overlap them
    colQty: {
        width: 28,
        textAlign: "center"
    },
...
function ReceiptPDF({ cartTotal, amount, change, customer, items }) {
    ...
    const ITEM_HEIGHT = 34; // give room for wrapped lines (increase if needed)
...
//# sourceMappingURL=%5Broot-of-the-server%5D__b07d1cfb._.js.map