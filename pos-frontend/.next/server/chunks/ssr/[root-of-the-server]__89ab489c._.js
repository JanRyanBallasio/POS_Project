...
;
const rawFromEnv = process.env.NEXT_PUBLIC_API_URL ?? ("TURBOPACK compile-time value", "http://3.25.180.232:5000/api") ?? '';
const browserFallback = ("TURBOPACK compile-time falsy", 0) ? "TURBOPACK unreachable" : '';
// normalize env: trim and remove surrounding quotes
const normalize = (v)=>v ? String(v).trim().replace(/^["']|["']$/g, '') : '';
...
const rawBase = ("TURBOPACK compile-time value", "http://3.25.180.232:5000/api") ?? "";
...
const useAddCategory = ()=>{
    ...
    const addCategory = async (payload, options)=>{
        ...
        const raw = process.env.NEXT_PUBLIC_API_URL ?? ("TURBOPACK compile-time value", "http://3.25.180.232:5000/api") ?? "";
...
const API_BASE_URL = (("TURBOPACK compile-time value", "http://3.25.180.232:5000/api") || "").replace(/\/$/, "");
...
const productApi = {
    ...
    async getByBarcode (barcode) {
        ...
        try {
            ...
            const qjson = await res2.json().catch(()=>({
                    data: []
                }));
...
function useAddProduct() {
    ...
    const addProduct = async (product)=>{
        ...
        try {
            ...
            const API_BASE = (("TURBOPACK compile-time value", "http://3.25.180.232:5000/api") || "").replace(/\/$/, "");
...
//# sourceMappingURL=%5Broot-of-the-server%5D__89ab489c._.js.map